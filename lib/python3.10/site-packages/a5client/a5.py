...
logging.basicConfig(filename= config.log['filename'], level=logging.DEBUG, format="%(asctime)s:%(levelname)s:%(message)s") #Tuve que modificar confil.logfile por config.log['filename'] por error de estructrua de datos
logging.FileHandler(config.log['filename'],"w+")

from a5client.schemas import schemas

serie_schema = open("%sschemas/yaml/serie.yml" % config.datadir)
